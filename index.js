...
global.uprlicencja = client.config.botConfig.uprlicencja;
...
global.permisjalicencji = message.guild.roles.cache.get(uprlicencja)
//---------------[REAKCJA NA PING]-----------------\\

//-----------------------------[AntyZaproszenia]-----------------------------\\
        let link = ["discord.gg", "discord.com/invite", "dsc.gg/"]
        if (link.some(word => message.content.toLowerCase().includes(word))) {
            message.delete()
            const embed3 = new Discord.MessageEmbed()
        
            .setDescription(`${emotki.nie} Na tym serwerze nie możesz wysyłać zaproszeń do innych discordów!`)
            .setColor(`RED`)
            .setFooter(`${message.author.tag} (${message.author.id})`, message.author.displayAvatarURL({ dynamic: true }))
